...
import { formatAmount, formatTime, currentMonth, monthRange, groupByDay } from '../utils';
...
const [recent, setRecent]     = useState<Transaction[]>([]);
  ...
  const load = () => {
    Promise.all([getSummary(month), getTransactions(monthRange(0)), getPersons()])
      .then(([s, txns, p]) => { setSummary(s); setRecent(txns); setPersons(p); setLoadError(false); })
  ...
  // Redirect to setup wizard on first login (no transactions + setup not done)
  useEffect(() => {
    if (loading) return;
    const setupKey = user?.sub ? `finsight_setup_${user.sub}` : null;
    if (setupKey && !localStorage.getItem(setupKey) && recent.length === 0 && !summary?.income) {
      navigate('/setup');
    }
  }, [loading, recent.length, summary, user?.sub]);
    ...
    <div className="min-h-screen bg-slate-50 pb-28">
      ...
      <div className="px-4 pt-4">
        <div className="rounded-3xl bg-slate-900 p-5 shadow-lg shadow-slate-900/10">
          ...
          {/* Income allocation: Expenses + Family + Lent (net) + Invested + Saved */}
          <AllocationBar income={income} expenses={spent} family={family} lent={lent} recovered={recovered} invested={invested} returns={returns} costBasisReturned={costBasisReturn} savings={saved} />
        </div>
      </div>
...
// Expenses, Family, net Lending, net Investing, and what was Saved. Lending and
// investing are shown net of money that came back, so the segments sum to income.
function AllocationBar({ income, expenses, family, lent, recovered, invested, returns, costBasisReturned, savings }: {
  income: number; expenses: number; family: number; lent: number; recovered: number; invested: number; returns: number; costBasisReturned: number; savings: number;
}) {
  ...
  return (
    <div className="mt-5">
      <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-2">
        How {formatAmount(income)} income was used
      </p>
